	...
	useEffect(() => {
		...
		const fetchVehicleDetails = async () => {
			try {
				const {
					success,
					vehicle: { number, type, model, maxPassengers },
				} = await vehicleService.fetchVehicle();
				if (success) {
					setVehicleType(type);
	...
	return (
		<div className="relative">
			<div className="text-center">
				<p className="text-[max(14px,1vw)] text-gray-500 font-medium">
					Step 1 of 3
				</p>
			...
			<p className="text-[max(14px)] font-extrabold sm:font-medium text-gray-500 mt-4">
				Select Vehicle Type
			</p>
			<div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-2">
				{VEHICLE_CATEGORIES.map(
					({ id, name, Icon, description, badge, maxPassengers }, index) => {
						...
						return (
							<motion.div
								...
								className={`rounded-2xl border p-2 pt-4 flex flex-col items-center gap-2 transition cursor-pointer ${
									isActive
							...
							>
								...
								<p
									className={`text-[max(12px)] font-black sm:font-semibold text-center ${isActive ? "text-gray-300" : "text-gray-400"}`}
								>
									{description}
								</p>
							</motion.div>
			...
			<div>
				<label
					...
					className="font-extrabold sm:font-medium text-gray-500 text-[14px]"
				>
					Vehicle Number{" "}
				</label>
				<input
					...
					className={`w-full mt-2 border-b pb-2 text-[max(16px)] focus:outline-none focus:border-background transition ${
						errors["number"]
							? "border-red-500 text-red-700"
							: "text-background border-gray-300"
					}`}
				/>
				{errors["number"] && (
