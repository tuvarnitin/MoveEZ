					...
					<AnimatePresence mode="sync">
						{links.map(({ title, to }, index) => (
							<motion.div key={index}>
								...
								>
									<NavLink
										className="w-full h-full flex py-3 px-4 items-center gap-2"
										to={to}
									>
										{title}
										{title === "Pending Requests" && (
											<span className="w-4 h-4 rounded-full bg-white text-xs text-background flex items-center justify-center font-semibold">
												{pendingRequestCount}
											</span>
										)}
									</NavLink>
